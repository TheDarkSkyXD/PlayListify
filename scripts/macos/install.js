// PlayListify - macOS Dependency Installer
// Installs yt-dlp and FFmpeg for macOS platforms
...
// Get yt-dlp URL based on architecture
...
// Get FFprobe URL based on architecture
function getFFprobeUrl() {
  return process.arch === 'arm64'
    ? 'https://evermeet.cx/ffmpeg/getrelease/ffprobe/zip/arm64'
...
async function extractZip(zipPath, extractDir) {
  c.step(`Extracting archive: ${zipPath}`);
...
async function installFFmpeg() {
  try {
    ...
    const ffmpegZipPath = path.join(FFMPEG_TEMP_DIR, 'ffmpeg.zip');
    
    c.step('Downloading FFmpeg...');
    await downloadFile(ffmpegUrl, ffmpegZipPath);
    
    // Extract the archive
    await extractZip(ffmpegZipPath, FFMPEG_TEMP_DIR);
    
    // Copy FFmpeg to bin directory
    const ffmpegTempPath = path.join(FFMPEG_TEMP_DIR, 'ffmpeg');
    const ffmpegBinPath = path.join(FFMPEG_BIN_DIR, 'ffmpeg');
    
    if (existsSync(ffmpegTempPath)) {
      c.step('Copying FFmpeg binary to bin directory...');
      fs.copyFileSync(ffmpegTempPath, ffmpegBinPath);
      chmodSync(ffmpegBinPath, '755');
      c.success('FFmpeg binary copied to bin directory');
    } else {
    ...
    const ffprobeZipPath = path.join(FFMPEG_TEMP_DIR, 'ffprobe.zip');
    
    c.step('Downloading FFprobe...');
    await downloadFile(ffprobeUrl, ffprobeZipPath);
    
    // Extract the archive
    await extractZip(ffprobeZipPath, FFMPEG_TEMP_DIR);
    
    // Copy FFprobe to bin directory
    const ffprobeTempPath = path.join(FFMPEG_TEMP_DIR, 'ffprobe');
    const ffprobeBinPath = path.join(FFMPEG_BIN_DIR, 'ffprobe');
    
    if (existsSync(ffprobeTempPath)) {
      c.step('Copying FFprobe binary to bin directory...');
      fs.copyFileSync(ffprobeTempPath, ffprobeBinPath);
      chmodSync(ffprobeBinPath, '755');
      c.success('FFprobe binary copied to bin directory');
    } else {
      c.warning('FFprobe binary not found in extracted files');
    }
    ...
    const ffmpegResult = spawnSync(ffmpegBinPath, ['-version'], {
      encoding: 'utf8',
      stdio: 'pipe'
    });
...
async function main() {
  ...
  console.log(`${colors.bright}${colors.blue}║           MACOS DEPENDENCY INSTALLER            ║${colors.reset}`);
  console.log(`${colors.bright}${colors.blue}╚══════════════════════════════════════════════════╝${colors.reset}`);
  console.log('\n');
  ...
  console.log('\n');
  c.success('macOS dependency installation completed successfully!');
  console.log('\n');

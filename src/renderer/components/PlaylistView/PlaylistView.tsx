...
import { Playlist, PlaylistVideoWithDetails } from '../../../shared/types';
...
export const PlaylistView: React.FC<PlaylistViewProps> = ({
  ...
  const [filteredVideos, setFilteredVideos] = useState<PlaylistVideoWithDetails[]>([]);
  ...
  // Handle drag start
  const handleDragStart = (video: PlaylistVideoWithDetails) => {
    setDraggedVideoId(video.id);
  };
  
  // Handle drag over
  const handleDragOver = (video: PlaylistVideoWithDetails) => {
  ...
  const handleDragEnd = () => {
    if (draggedVideoId !== null && dragOverVideoId !== null && data) {
      const draggedVideo = data.videos.find((v) => v.id === draggedVideoId);
      const dragOverVideo = data.videos.find((v) => v.id === dragOverVideoId);

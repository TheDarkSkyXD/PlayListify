...
// Fix SQLite3 native module loading issue
if (process.env.NODE_ENV !== 'development') {
  try {
    // Get the correct sqlite3 path
    const appRoot = app.getAppPath();
    const modulePath = path.join(path.dirname(appRoot), 'node_modules', 'sqlite3');
    
    // Force the sqlite3 module to look in the right place for its binary
    // @ts-ignore - Adding custom property to the module for path resolution
    sqlite3.path = modulePath;
    
    logger.info(`Set SQLite3 module path to: ${modulePath}`);
  } catch (error) {
    logger.error('Failed to set SQLite3 module path:', error);
  }
}
...
class Database {
  ...
  constructor(dbPath: string) {
    ...
    try {
      ...
      logger.error(`Failed to open SQLite database at ${dbPath}:`, error);
      throw new Error(`SQLite3 database initialization failed: ${error.message}`);
    }
  }

...
import { checkVideoStatus } from '../services/ytdlpService';
...
export const registerPlaylistHandlers = () => {
  ...
  // Add handler for checking video status (private/deleted)
  ipcMain.handle(IPC_CHANNELS.CHECK_VIDEO_STATUS, async (event, { videoId }) => {
    try {
